...
namespace BVE5Language.Resolver
{
	...
	/*public class OverloadResolution
	{
		sealed class Candidate
		{
			...
			public Candidate(IParameterizedMember member)
			{
				this.Member = member;
		...
		readonly string[] argument_names;
		...
		#region Constructor
		public OverloadResolution(ICompilation compilation, ResolveResult[] arguments, string[] argumentNames = null, IType[] typeArguments = null)
		{
			if(compilation == null)
			...
			this.argument_names = argumentNames;
		...
		public OverloadResolutionErrors AddCandidate(IParameterizedMember member, OverloadResolutionErrors additionalErrors)
		{
			...
			{
				Candidate expanded_candidate = new Candidate(member, true);
		...
		void RunTypeInference(Candidate candidate)
		{
			...
			IList<IType> class_type_args = (parameterizedDeclaringType != null) ? parameterizedDeclaringType.TypeArguments : null;
			
			TypeInference ti = new TypeInference(compilation, conversions);
			bool success;
			candidate.InferredTypes = ti.InferTypeArguments(candidate.TypeParameters, arguments, candidate.ParameterTypes, out success, class_type_args);
			if(!success)
				candidate.AddError(OverloadResolutionErrors.TypeInferenceFailed);

			// Now substitute in the formal parameters:
			var substitution = new ConstraintValidatingSubstitution(class_type_args, candidate.InferredTypes, this);
		...
		IList<ResolveResult> GetArgumentsWithConversions(ResolveResult targetResolveResult, IParameterizedMember bestCandidateForNamedArguments)
		{
			...
			for (int i = 0; i < args.Length; i++) {
				...
				if (bestCandidateForNamedArguments != null && argument_names[i] != null) {
					...
					if (parameterIndex >= 0) {
						...
						argument = new NamedArgumentResolveResult(argument_names[i], argument);
					}
				}
				args[i] = argument;
			}
